...
Click any figure to open a larger version; click the enlarged image to close
it.
...
modes. Each class uses one saved approximate-neighbor graph.
...
data and saved graph, changing only to normalized LTSA. The normalized modes
...
the retained span is represented across observations. Here, block and per-mode
support tell the same story without treating any particular eigenvector basis
as uniquely meaningful.
...
Set `FLOTSAM_SPECTRAL_BLOCK_FIGURE_DIR` to a directory to save the five article
diagnostics as PNG files; otherwise they draw on the current device.
