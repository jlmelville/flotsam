...
At `k = 7`, mixing objects produces a disconnected neighbor graph. We fit each
object separately and use objects 4 and 1, whose connected graphs mostly follow
the rotation. Each fit uses ordinary LTSA with `ndim = 1`, exact neighbors, and
four retained modes. The coordinates come from pixels; pose supplies the point
colors and thumbnail labels.
...
It ranges from `1 / n` when one observation carries the mode to `1` when the
energy is spread uniformly. Here, a mode's energy means its squared coordinate
values, `v^2`:
...
Fashion-MNIST supplies a different outcome. We fit trousers, dresses, and bags
separately with ordinary LTSA, using `ndim = 2`, `k = 15`, and four retained
modes. Each class uses one saved approximate-neighbor graph.

All twelve retained coordinates lie near the single-observation support limit,
and the largest 1% of observations carry more than 99.96% of each coordinate's
energy. The graphs are connected, local patches have full rank, and the
eigensolves converge. The support plot therefore identifies localization as
...
modes are. Fashion-MNIST has weak boundaries and localized alternatives.
...
weight to place on each coordinate. Fashion-MNIST shows that retaining modes
also diagnoses a localized low spectrum.
...
Roland Vollgraf). Static article figures contain low-resolution thumbnails;
the reproduction script downloads each dataset through `snedata`.
